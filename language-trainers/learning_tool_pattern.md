...
- **Session / round / review loop**: a round asks every (word, enabled drill type) combination in the selection once each, no repeats -- a word with translation and conjugation both enabled gets both, not just one -- then stops at an **end-of-round summary**; missed items feed a **review round that re-drills exactly those questions**, and once a review clears to 100%, it can **retest the whole original round** (right answers included) rather than only offering a fresh, unrelated one. Keyboard-first (Enter submits / advances). (Currently applied to the Polish app; the others still use an open-ended weighted pool with a soft repeat penalty rather than a hard per-round exclusion.)
