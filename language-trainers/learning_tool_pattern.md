...
- **Filtering**: by **lesson** (L1–L15 + extras) and by mode; a live "N words · Q questions in selection" count -- Q counts every (word, enabled drill type) pair the round would actually ask, which is what a round now equals exactly (see the round/review loop entry above), not just the vocabulary size.
...
- **Session / round / review loop**: a round asks every (word, enabled drill type) combination in the selection once each, no repeats -- a word with translation and conjugation both enabled gets both, not just one -- then stops at an **end-of-round summary**; missed items feed a **review round that re-drills exactly those questions**, and once a review clears to 100%, it can **retest the whole original round** (right answers included) rather than only offering a fresh, unrelated one. Keyboard-first (Enter submits / advances). (The no-repeat round is shipped on all eight apps; "retest the whole round" is Polish-only so far.)
